...
function itm(value)
{
	...
	window.location.href="remove.php?itms="+value+"&itm="+z+"&m="+x;
}
...

{
	...
	x.selected=true;
	window.location.href="remove.php?itm="+value;
}
function sub()
{
	...
	window.location.href="remove.php?tp="+tp+"&rum="+rum+"&items="+items+"&itmid="+itmid;
}
</script>
</head>
<body>
...
echo "<select id='lvl' onchange='chng(this.value);' hidden>";
...
if(isset($_GET["tp"]))
{
	...
	$c=$_GET["items"];
	$d=$_GET["itmid"];
	$qry1=mysqli_query($db,"UPDATE $c SET room='0' WHERE id='$d'");
	...
	echo "alert('Item Removed Successfully')";
	}
